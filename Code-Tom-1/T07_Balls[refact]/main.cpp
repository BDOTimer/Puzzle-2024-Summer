...
/// Трек.
///----------------------------------------------------------------------------:
struct  Track   : sf::Drawable
{       Track() : m(3)
        {
            for(auto& e : m) r.push_back(const_cast<RenderSprite*>(&e));
        }
...
protected:
          std::list  <RenderSprite*> r;
    const std::vector<RenderSprite > m;

    Trajectory* trajectory;

    float a = 0.5f;

    void setPosition(const size_t i)
    {   /// ...
    }
    ...
    {
        for(const auto& e : r) {e->rotate(-a); target.draw(*e , states);}
    }
};
...
private:
    ...
    std::array<Track*, 2> tracks{
        static_cast<Track*>(&tL),
        static_cast<Track*>(&tR)
    };

    virtual void draw(sf::RenderTarget& target,
                      sf::RenderStates  states) const
    {
        for(const auto& e : tracks) target.draw(*e, states);
...
{       SceneGame() : TestA({690,690}, {0, 0}, {0,0,255,100})
        {
            name = "SceneGame";
            text.setString(std::string(name));
...
///----------------------------------------------------------------------------:
struct  RenderLoop
{       RenderLoop() : window(sf::VideoMode(
                            ConfigGame::p->winsize.x,
                            ConfigGame::p->winsize.y), L"Tutor-07::Board"),
            fps  (cam_ui)
        {
